...
class Trainer(object):
    ...
    def forward(self, color_heightmap, depth_heightmap, is_volatile=False, specific_rotation=-1):
        # 如果输入是 CUDA tensor,先移到 CPU
        if isinstance(color_heightmap, torch.Tensor):
            if color_heightmap.is_cuda:
                color_heightmap = color_heightmap.cpu()
            color_heightmap = color_heightmap.numpy()
        if isinstance(depth_heightmap, torch.Tensor):
            if depth_heightmap.is_cuda:
                depth_heightmap = depth_heightmap.cpu()
            depth_heightmap = depth_heightmap.numpy()

        # 确保输入维度正确
        if len(depth_heightmap.shape) == 3:
            depth_heightmap = depth_heightmap.squeeze(0)
        if len(color_heightmap.shape) == 4:
            color_heightmap = color_heightmap.squeeze(0)
        ...
        print(f"Depth map range: {depth_min:.4f} - {depth_max:.4f}")
        ...
        if self.use_cuda:
            # 确保使用to(device)将张量移动到模型所在设备
            input_color_data = input_color_data.to(device)
            input_depth_data = input_depth_data.to(device)
            print(f"输入数据已移动到 {device} 设备")
            
            # 再次检查模型是否完全在CUDA上
            cpu_params = [name for name, param in self.model.named_parameters() if not param.is_cuda]
            if len(cpu_params) > 0:
                print(f"警告: 以下参数不在CUDA上: {cpu_params}")
                # 尝试再次将模型移动到CUDA
                self.model = self.model.to(device)
            else:
                print("模型参数检查正常，所有参数都在CUDA上")
        ...
        try:
            ...
            print(f"前向传播发生错误: {e}")
            # 打印更多错误信息
            print(f"输入数据设备: {input_color_data.device}")
            param_device = next(self.model.parameters()).device
            print(f"模型参数设备: {param_device}")
            # 如果设备不匹配，尝试解决
            if str(input_color_data.device) != str(param_device):
                print("设备不匹配，尝试修复...")
    ...
    def get_label_value(self, primitive_action, push_success, grasp_success, change_detected, push_predictions, grasp_predictions, next_color_heightmap, next_depth_heightmap):
        ...
        print(f"Action type: {primitive_action}")
        ...
        if primitive_action == 'push':
            # 推动动作奖励逻辑增强
            if push_success:
                print("Push action successful, setting high reward")
                current_reward = 0.5  # 提高推动成功的奖励
            elif change_detected:
                print("Scene change detected, setting base reward")
                current_reward = 0.2
            else:
                print("Push had no effect, setting zero reward")
                current_reward = 0.0
        elif primitive_action == 'grasp':
            if grasp_success:
                print("Grasp action successful, setting high reward")
                current_reward = 1.0
                # 在测试模式下立即返回高奖励，不考虑未来奖励
                if hasattr(self, 'is_testing') and self.is_testing:
                    print("Test mode: Directly returning grasp success reward: 1.0")
                    return 1.0, 1.0
            else:
                print("Grasp failed, setting zero reward")
                current_reward = 0.0

        # 计算未来奖励
        future_reward = 0.0
        try:
            # 仅当当前奖励不是最大值(1.0)时才计算未来奖励
            if current_reward < 1.0:
                next_push_predictions, next_grasp_predictions, next_state_feat = self.forward(next_color_heightmap,
                                                                                           next_depth_heightmap, is_volatile=True)
                
                # 确保预测值在CPU上
                if isinstance(next_push_predictions, torch.Tensor):
                    next_push_predictions = next_push_predictions.detach().cpu().numpy()
                if isinstance(next_grasp_predictions, torch.Tensor):
                    next_grasp_predictions = next_grasp_predictions.detach().cpu().numpy()
                
                # 计算最大可能的未来奖励
                future_reward = max(np.max(next_push_predictions), np.max(next_grasp_predictions))
                print(f'Calculating future reward: {future_reward:.4f}')
            else:
                print('Current reward is already maximum, not calculating future reward')
        except Exception as e:
            print(f"Error calculating future reward: {e}")
            future_reward = 0.0

        print('Current reward: %f' % (current_reward))
        print('Future reward: %f' % (future_reward))
        expected_reward = current_reward + self.future_reward_discount * future_reward
        print('Expected total reward: %f + %f x %f = %f' % (current_reward, self.future_reward_discount, future_reward, expected_reward))
        
        # 强制设置最小奖励阈值，确保动作成功有明显奖励
        if primitive_action == 'grasp' and grasp_success and expected_reward < 0.7:
            expected_reward = 0.7
            print("Enforcing minimum reward for successful grasp: 0.7")
        elif primitive_action == 'push' and push_success and expected_reward < 0.4:
            expected_reward = 0.4
            print("Enforcing minimum reward for successful push: 0.4")
    ...
    def backprop(self, color_heightmap, depth_heightmap, primitive_action, best_pix_ind, label_value):
        # 获取当前模型设备
        device = next(self.model.parameters()).device
        print(f"backprop方法中，当前模型设备: {device}")
        ...
        # 移动数据到正确设备
        if color_heightmap.device != device:
            color_heightmap = color_heightmap.to(device)
        if depth_heightmap.device != device:
            depth_heightmap = depth_heightmap.to(device)
        ...
        if self.method == 'reinforcement':
            try:
                ...
                label_weights_tensor = torch.from_numpy(label_weights).float().to(device)
                ...
                # 确定动作类型的字符串表示
                if isinstance(primitive_action, int):
                    primitive_action_str = 'grasp' if primitive_action == 1 else 'push'
                else:
                    primitive_action_str = primitive_action
                ...
                if primitive_action_str == 'push':
                    try:
                        ...
                        # 确保张量在正确的设备上
                        push_output = model_output[0][0].to(device)
                ...
                elif primitive_action_str == 'grasp':
                    try:
                        # 确保索引有效
                        ...
                        # 确保张量在正确的设备上
                        grasp_output = model_output[0][1].to(device)
