...
class Experiment:
    ...
    def __init__(self):
        ...
        self.primitive_action = 0  # 0: PUSH, 1: GRASP, 默認為0
        self.best_pix_ind = (0, 0, 0)  # 預設值(rotation_idx, pixel_y, pixel_x)
        self.best_rotation_angle = 0.0
        self.best_pix_ind_coords = (0, 0)  # 預設值(pixel_x, pixel_y)
        ...
        # 添加动作记录跟踪
        self.recent_push_locations = []  # 存储最近的推动位置
        self.recent_grasp_locations = []  # 存储最近的抓取位置
        self.consecutive_failures = 0  # 连续失败次数
        self.action_memory_size = 5  # 记忆最近的动作数量
        self.location_similarity_threshold = 20  # 像素距离阈值，判断动作是否在相似位置
    
    def set_primitive_action(self, action_mode):
        """設置動作類型，確保類型安全
        
        Args:
            action_mode: ActionMode.PUSH 或 ActionMode.GRASP
        """
        self.primitive_action = int(action_mode)  # 轉換為整數以避免類型問題
    ...
    def set_predictions(self, push_preds, grasp_preds):
        ...
        """
        # 確保numpy數組類型
        if push_preds is not None:
            self.push_predictions = np.array(push_preds)
        if grasp_preds is not None:
            self.grasp_predictions = np.array(grasp_preds)
    
    def set_best_pix_ind(self, pix_ind):
        """設置最佳像素索引
        
        Args:
            pix_ind: 像素索引元組(rot_idx, y, x)或(y, x)
        """
        if len(pix_ind) == 3:
            self.best_pix_ind = tuple(map(int, pix_ind))
        elif len(pix_ind) == 2:
            # 如果只提供(y, x)，預設旋轉索引為0
            self.best_pix_ind = (0, int(pix_ind[0]), int(pix_ind[1]))
    ...
    def set_coords(self, coords):
        ...
        Args:
            coords: 坐標元組(x, y)
        """
        self.best_pix_ind_coords = tuple(map(int, coords))
    ...
    def set_rotation_angle(self, angle):
        ...
        Args:
            angle: 旋轉角度（弧度）
        """
        self.best_rotation_angle = float(angle)
    
    def reset(self):
        """重置實驗狀態（保留迭代計數）"""
        self.push_success = False
        self.grasp_success = False
        # 不重置primitive_action和坐標，因為它們可能被接下來的代碼使用
        self.change_detected = False
        # 重置物体位置跟踪
        self.prev_object_position = None
        
        # 重置动作记录跟踪
        self.recent_push_locations = []
        self.recent_grasp_locations = []
    ...
    def add_action_to_memory(self, action_type, pixel_location):
        ...
        Args:
            action_type: ActionMode.PUSH 或 ActionMode.GRASP
            pixel_location: 动作像素位置 (x, y)
        """
        if action_type == ActionMode.PUSH:
            self.recent_push_locations.append(pixel_location)
            # 只保留最近的n个位置
            if len(self.recent_push_locations) > self.action_memory_size:
                self.recent_push_locations.pop(0)
        else:  # GRASP
            self.recent_grasp_locations.append(pixel_location)
            if len(self.recent_grasp_locations) > self.action_memory_size:
                self.recent_grasp_locations.pop(0)
    ...
    def is_action_repeated(self, action_type, pixel_location):
        ...
        Returns:
            bool: 是否为重复动作
        """
        locations = self.recent_push_locations if action_type == ActionMode.PUSH else self.recent_grasp_locations
        ...
        for loc in locations:
            dist = np.sqrt((loc[0] - pixel_location[0])**2 + (loc[1] - pixel_location[1])**2)
            if dist < self.location_similarity_threshold:
                return True
    ...
    def get_exploration_boost(self):
        ...
        Returns:
            float: 探索率提升值 (0.0-0.5)
        """
        # 连续失败越多，探索率提升越大
        boost = min(0.5, self.consecutive_failures * 0.1)
        return boost
        ...
        if self.state[0] == RobotState.IDLE:
            ...
            if self.wait_time[0] < self.wait_time_push:
                ...
                if progress_float <= 0.3:
                    ...
                    # 打印当前使用的推动高度
                    if int(progress_float * 100) % 20 == 0:  # 每20%的进度打印一次
                        height_above_surface = pos_z - workspace_limits[2][0]
                        print(f"推动阶段 - 当前高度: {pos_z:.4f}，距离表面: {height_above_surface*1000:.1f}mm")
            ...
            if not executing_action:
                try:
                    ...
                    if np.isnan(depth_img).any():
                        print(f"注意: 深度圖包含 {np.sum(np.isnan(depth_img))} 個NaN值")
