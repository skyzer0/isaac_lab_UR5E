...
class reinforcement_net(nn.Module):
    ...
    def __init__(self, use_cuda): # , snapshot=None
        ...
        # 保存到类变量中的数据是否强制保存到GPU上
        self.force_save_to_gpu = True
    ...
    def _verify_device(self):
        """验证所有模型组件是否都在正确的设备上"""
        # 确保目标设备是GPU（如果use_cuda为True）
        if self.use_cuda and 'cuda' not in str(self.device):
            self.device = torch.device("cuda")
            print(f"强制使用CUDA设备: {self.device}")
        
        # 确保所有组件在正确的设备上
        self.to(self.device)  # 先将整个模型移到设备上
        ...
        for component, name in components:
            try:
                ...
                # 改进设备比较：只要都是CUDA设备就认为是一致的
                if self.use_cuda:
                    if 'cuda' not in str(component_device):
                        issues.append(f"{name} on {component_device}, should be on CUDA")
                        # 自动修复
                        component.to(self.device)
                else:
                    # 非CUDA情况，需要完全匹配
                    if component_device != self.device:
                        issues.append(f"{name} on {component_device}, should be on {self.device}")
                        # 自动修复
                        component.to(self.device)
            except StopIteration:
                print(f"信息: {name} 没有参数，跳过检查")
        ...
        if issues:
            ...
            if remaining_issues:
                print(f"警告: 以下组件仍然不在正确的设备上: {', '.join(remaining_issues)}")
        else:
            print(f"设备检查通过: 所有组件都在正确的设备上")
        
        return self.device
        
    def ensure_on_gpu(self, tensor_or_list):
        """确保张量或张量列表在GPU上（如果启用了CUDA）"""
        if not self.use_cuda or not self.force_save_to_gpu:
        ...
        if isinstance(tensor_or_list, list):
            ...
            for item in tensor_or_list:
                if isinstance(item, list):
                    # 嵌套列表
                    result.append(self.ensure_on_gpu(item))
            ...
            # 单个张量
            if tensor_or_list.device != self.device:
    ...
    def forward(self, input_color_data, input_depth_data, is_volatile=False, specific_rotation=-1):
        ...
        print(f"模型Forward方法中，设备: {device}")
        ...
        if is_volatile:
            with torch.no_grad():
                ...
                # 保存在类变量中以便backprop时使用 - 确保数据在GPU上
                self.output_prob = self.ensure_on_gpu(output_prob)
                self.interm_feat = self.ensure_on_gpu(interm_feat)
                state_feat_buffer = self.ensure_on_gpu(state_feat_buffer)

                print(f"前向传播结束，输出设备: {output_prob[0][0].device if output_prob and isinstance(output_prob[0], list) else 'unknown'}")
            ...
            # 保存中间特征 - 确保数据在GPU上
            self.interm_feat.append(self.ensure_on_gpu([interm_push_feat, interm_grasp_feat]))
            ...
            # 保存输出概率 - 确保数据在GPU上
            self.output_prob.append(self.ensure_on_gpu([push_output, grasp_output]))

            print(f"训练模式前向传播结束，输出设备: {self.output_prob[0][0].device if self.output_prob and isinstance(self.output_prob[0], list) else 'unknown'}")
            
            # 返回三个值：输出概率、中间特征、状态特征 - 所有数据都在GPU上
            return self.ensure_on_gpu(self.output_prob), self.ensure_on_gpu(self.interm_feat), self.ensure_on_gpu(interm_push_feat)
