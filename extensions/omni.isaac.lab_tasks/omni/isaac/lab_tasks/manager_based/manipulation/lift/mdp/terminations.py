...
def joint_pos_out_of_limit(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    # 关节数据形状
    print(f"joint_pos shape: {asset.data.joint_pos.shape}")
    print(f"soft_joint_pos_limits shape: {asset.data.soft_joint_pos_limits.shape}")
    ...
    print(f"out_of_upper_limits shape: {out_of_upper_limits.shape}")
    print(f"out_of_lower_limits shape: {out_of_lower_limits.shape}")
